            ...
            <Link href={`/projects/${project.id}`} className="flex-1 block ml-2">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-gray-600 dark:text-gray-300 text-sm mt-1">{project.github_url || 'No repository URL'}</p>
                    </div>
                </div>
                <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <span>Last update: {timeAgo(project.updated_at)}</span>
                </div>
            </Link>
...
export default function Dashboard() {
    ...
    const fetchAccessToken = async () => {
        ...
        if (!res.ok) {
          throw new Error('Failed to fetch access token. Please re-login.');
        }
        const data = await res.json();
        return data.accessToken;
    };

    const fetchProjects = async (current_user: any, sortBy: string = 'newest') => {
        if (!current_user?.sub) return;
        setIsProjectsLoading(true);
        setApiError(null);
        try {
            const accessToken = await fetchAccessToken();
            const data: Project[] = await getProjects(accessToken, current_user.sub, sortBy);
            setProjects(data);
        } catch (err: any) {
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        try {
            const accessToken = await fetchAccessToken();
            await createProject(accessToken, projectData);
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            if (user?.sub) {
                try {
                    const accessToken = await fetchAccessToken();
                    await fetch(`${apiBaseUrl}/api/projects/order`, {
                        method: 'PATCH',
                        headers: { 
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${accessToken}`
                    ...
                    alert("順序の保存に失敗しました。");
                    fetchProjects(user); // Revert to original order from server
                }
            }
        }
    };

    const handleAutoSort = (sortKey: string) => {
        if (!user?.sub) return;
        fetchProjects(user, sortKey);
    };
    ...
    return (
        <div className="bg-gray-50 dark:bg-black min-h-screen">
            ...
            <div className="container mx-auto p-4 md:p-8">
                ...
                {apiError && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert"><strong className="font-bold">Error: </strong><span className="block sm:inline">{apiError}</span></div>}
                        ...
                        <div className="flex flex-col md:flex-row gap-10">
                            <div className="w-full md:w-1/3">
                                ...
                                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="url" id="githubUrl" value={githubUrl} onChange={(e) => setGithubUrl(e.target.value)} className="mt-1 p-2 w-full border rounded-md bg-gray-100 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-200" placeholder="https://github.com/user/repo"/>
                                    </div>
                            ...
                            <div className="w-full md:w-2/3 md:border-l md:border-gray-200 dark:md:border-gray-700 md:pl-10">
                                ...
                                <div className="space-y-4">
                                    {isProjectsLoading ? (
                                        <div className="text-center py-10"> <p className="text-gray-500 dark:text-gray-400">プロジェクト一覧を読み込んでいます...</p> </div>
                                    ) : projects.length > 0 ? (
